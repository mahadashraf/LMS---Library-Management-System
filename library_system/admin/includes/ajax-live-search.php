...
$sql = "SELECT * FROM lms_category WHERE Category_name LIKE '%{$search_value}%' ";
...
$output .= "<tr><td>{$row["Category_name"]}</td><td>{$row["Category_status"]}</td><td>{$row["created_on"]}</td><td>{$row["updated_on"]}</td></tr>"; 
}
$output .= "</thead> </table>";
